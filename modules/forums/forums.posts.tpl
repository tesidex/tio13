...
<!-- BEGIN: FORUMS_POSTS_ADMIN -->
    <button type="button" class="btn btn-info" data-toggle="collapse" data-target="#admin-forum-options">
      {PHP.L.blogster_moderation}&nbsp;&nbsp;&nbsp;<i class="icon-circle-arrow-down icon-white"></i>
    </button>
...
<!-- END: FORUMS_POSTS_ADMIN -->
    <table class="table table-striped">
      <thead>
        <th class="coltop width20">{PHP.L.Author}</th>
        <th class="coltop width80">{PHP.L.Message}</th>
      </thead>
      <!-- BEGIN: FORUMS_POSTS_ROW -->
      <tr>
        <td class="centered">
          {FORUMS_POSTS_ROW_ANCHORLINK}
          <h4 class="centered">{FORUMS_POSTS_ROW_USERNAME}</h4>
          <p class="centered">
...
<!-- BEGIN: FORUMS_POSTS_NEWPOST -->
    ...
    <form action="{FORUMS_POSTS_NEWPOST_SEND}" method="post" name="newpost">
      <table class="flat">
        <tr>
          <td>{FORUMS_POSTS_NEWPOST_TEXT}{FORUMS_POSTS_NEWPOST_MYPFS}</td>
        </tr>
        <tr>
          <td class="centered"><button type="submit" class="btn btn-primary">{PHP.L.Reply}</button></td>
        </tr>
      </table>
    </form>
